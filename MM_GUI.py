...
'''
dirs1 = ['ARTAK_MM/LOGS', 'ARTAK_MM/POST/Lidar', 'ARTAK_MM/DATA/Processing', 'ARTAK_MM/DATA/Generated_Mesh',
         'ARTAK_MM/DATA/Generated_PointClouds', 'ARTAK_MM/DATA/Scan_Logs']
...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.tiles_radio_button3 = customtkinter.CTkRadioButton(self.fourth_frame,
                                                                text="Tiled PointCloud (Experimental)",
                                                                variable=self.mesh_from_pointcloud_type_var,
        ...
        self.appearance_mode_label = customtkinter.CTkLabel(self.fourth_frame, text="App Screen Mode")
        self.appearance_mode_label.grid(row=8, column=0, padx=20, pady=20, sticky="s")
        ...
        self.upload_yes_radio_button = customtkinter.CTkRadioButton(self.fourth_frame, text="N",
                                                                    variable=self.upload_var,
                                                                    value="N")
        self.upload_yes_radio_button.grid(row=12, column=1, padx=20, pady=10)

        self.upload_no_radio_button = customtkinter.CTkRadioButton(self.fourth_frame, text="Y",
                                                                   variable=self.upload_var,
                                                                   value="Y")
        ...
        self.yes_radio_button = customtkinter.CTkRadioButton(self.fourth_frame, text="N",
                                                             variable=self.delete_after_transfer_var,
                                                             value="N")

        self.yes_radio_button.grid(row=14, column=1, padx=20, pady=10)

        self.no_radio_button = customtkinter.CTkRadioButton(self.fourth_frame, text="Y",
                                                            variable=self.delete_after_transfer_var,
                                                            value="Y")
        ...
        self.save_settings = customtkinter.CTkButton(self.fourth_frame, text="Save Settings",
                                                     command=self.read_settings_and_save,
                                                     state="normal")
        ...
        self.local_radio_button = customtkinter.CTkRadioButton(self.home_frame, text="Local Drive",
                                                               variable=self.file_origin,
                                                               value="L")

        self.local_radio_button.place(x=50, y=25)

        self.exyn_sensor_radio_button = customtkinter.CTkRadioButton(self.home_frame, text="Exyn Sensor",
                                                                     variable=self.file_origin,
                                                                     value="R")
        ...
        self.browse_button_pc = customtkinter.CTkButton(self.home_frame, text="Browse", command=self.pre_processing,
                                                        state="normal")
    ...
    def display_remote_dir_listing(self):
        ...
        def download_threaded(selected_option):
            ...
            log_entry = str(
                datetime.now().strftime("%Y-%m-%d %H:%M:%S")) + " INFO Downloading Exlog " + selected_option + ".\r"
            self.write_to_runtime_log(log_entry)
            ...
            with open("ARTAK_MM/DATA/Scan_logs/" + new_pc_file, "wb") as exlog_to_save:
                ...
                for data in response.iter_content(block_size):
                    ...
                    log_entry = str(datetime.now().strftime("%Y-%m-%d %H:%M:%S")) + " INFO " + str(
                        percentage) + "% of " + str(selected_option) + " downloaded."
                    self.write_to_runtime_log(log_entry)
        ...
        self.listbox = tk.Listbox(remote_window, width=40, height=15, font=customtkinter.CTkFont(size=18),
                                  selectmode=tk.BROWSE, exportselection=False, yscrollcommand=scrollbar.set)
        ...
        self.local_radio_button = customtkinter.CTkRadioButton(remote_window, text="Download Only",
                                                               variable=self.download_type,
                                                               value="d")

        self.local_radio_button.place(x=10, y=305)

        self.exyn_sensor_radio_button = customtkinter.CTkRadioButton(remote_window, text="Download and Process",
                                                                     variable=self.download_type,
                                                                     value="dp")
    ...
    def open_obj(self, path):
        ...
        o3d.visualization.draw([{"name": "Model", "geometry": model, "material": material}],
                               title="ARTAK 3D Map Maker || LiDAR Edition", bg_color=[0, 0, 0, 0], show_skybox=False,
                               actions=None, width=1280, height=900)
