...
class pc2mesh:
    ...
    def process_exlog(self, exlog_path):
        ...
        with open("\\\wsl.localhost/Ubuntu-22.04\\home\\mapmaker\\exyn\\runtime.log", "r") as log_file:
            ...
            if 'This logfile is not indexed, please use --reindex to fix it' in logs:
                ...
                dest_after_xcloud = os.path.join(os.getcwd(),
                                                 "ARTAK/_MM/DATA/Generated_PointClouds/" + xcloud_file_copied)
    ...
    def get_file(self):
        ...
        logfilename = designator + filename.replace('.ply', '').replace('.pts', '').replace('.obj', '').replace('.e57',
                                                                                                                '').replace(
            '.ex', '')
        ...
        log_name = designator + filename.replace('.ply', '').replace('.pts', '').replace('.obj', '').replace('.e57',
                                                                                                             '').replace(
            '.ex', '') + "_" + str(d) + "_" + str(ct) + ".log"

        # Derive destination folders from source path
        post_dest_folder = "ARTAK_MM/POST/Lidar" + separator + pc_folder.replace("m_m_", "m_").replace("tm_tm_",
                                                                                                       "tm_") + separator + "Data"
        model_dest_folder = "ARTAK_MM/POST/Lidar" + separator + pc_folder.replace("m_m_", "m_").replace("tm_tm_",
                                                                                                        "tm_") + separator + "Data/Model"
        mesh_output_folder = "ARTAK_MM/DATA/Processing/" + folder_type + separator + pc_folder.replace("m_m_",
                                                                                                       "m_").replace(
            "tm_tm_", "tm_") + separator + "mesh" + folder_suffix
        simplified_output_folder = "ARTAK_MM/DATA/Processing/" + folder_type + separator + pc_folder.replace("m_m_",
                                                                                                             "m_").replace(
            "tm_tm_", "tm_") + separator + "simplified" + folder_suffix
        gen_path_folder = "ARTAK_MM/DATA/Generated_Mesh"
        with_texture_output_folder = "ARTAK_MM/DATA/Processing/" + folder_type + separator + pc_folder.replace("m_m_",
                                                                                                               "m_").replace(
            "tm_tm_", "tm_") + separator + "final" + folder_suffix
        log_folder = "ARTAK_MM/LOGS/"
        zip_file_for_compression = designator + filename.replace(".ex", ".zip").replace(".obj", ".zip").replace(".ply",
                                                                                                                ".zip").replace(
            ".pts", ".zip")
    ...
    def compute_normals_and_generate_mesh(self, downpcd, mesh_depth, texture_size):
        ...
        mesh = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(downpcd, depth=mesh_depth, width=0, scale=1.1,
                                                                         linear_fit=False)[0]
        generated_mesh = mesh_output_folder + separator + designator + filename.replace('.ply', '.obj').replace('.pts',
                                                                                                                '.obj').replace(
            '.ex', '.obj').replace('.e57', '.obj')
        ...
        try:
            ...
            if 'obj' in origin:
                ...
                try:
                    ...
                    newpath = simplified_output_folder + separator + designator + filename.replace('.ply',
                                                                                                   '.obj').replace(
                        '.pts', '.obj').replace('.ex', '.obj').replace('.e57', '.obj')
                    newpath_gen = gen_path_folder + separator + filename.replace('.ply', '.obj').replace('.pts',
                                                                                                         '.obj').replace(
                        '.ex', '.obj').replace('.e57', '.obj')
                    ...
                    try:
                        ...
                        newpath = simplified_output_folder + separator + filename.replace('.ply', '.obj').replace(
                            '.pts', '.obj').replace('.ex', '.obj').replace('.e57', '.obj')
                        newpath_gen = gen_path_folder + separator + filename.replace('.ply', '.obj').replace('.pts',
                                                                                                             '.obj').replace(
                            '.ex', '.obj').replace('.e57', '.obj')
                        ...
                        try:
                            ...
                            # Announce error and terminate.
                            messagebox.showerror('ARTAK 3D Map Maker',
                                                 'Could not compute Mesh from PointCloud. Aborting.')
            ...
            if f_number > face_number:  # Decimate over 6500000 faces (approx.950Mb)
                ...
                newpath1 = simplified_output_folder + separator + 'decimated_' + designator + filename.replace('.ply',
                                                                                                               '.obj').replace(
                    '.pts', '.obj').replace('.ex', '.obj').replace('.e57', '.obj')
    ...
    def add_texture_and_materials(self, newpath, newpath1, texture_size):
        ...
        newpath_texturized = with_texture_output_folder + separator + designator + filename.replace('.ply',
                                                                                                    '.obj').replace(
            '.pts', '.obj').replace('.ex', '.obj').replace('decimated_', '').replace('.e57', '.obj')
        ...
        zip_file = with_texture_output_folder + separator + designator + filename.replace('.obj', '').replace('.ply',
                                                                                                              '').replace(
            '.pts', '').replace('.ex', '').replace('.e57', '') + '.zip'
    ...
    def compress_into_zip(self, with_texture_output_folder, newpath, zip_file):
        ...
        if resulting_mesh_type == "tm":
            ...
            target_to_tile = with_texture_output_folder + separator + designator + filename.replace('.ply',
                                                                                                    '.obj').replace(
                '.pts', '.obj').replace('.ex', '.obj').replace('.e57', '.obj')
            ...
            cmd = "Obj2Tiles --lods 1 --divisions 1 --lat " + str(lat) + " --lon " + str(lon) + " --alt " + str(
                elev) + " " + target_to_tile + " " + tiles_folder + " --keeptextures > " + log_folder + "runtime.log"
            ...
            logging.info(
                'Compressing to ' + tiles_folder + separator + designator + filename.replace('.ply', '.zip').replace(
                    '.pts', '.zip').replace('.ex', '.zip').replace('.obj', '.zip').replace('.e57', '.zip') + ".")
            message = 'INFO Compressing to ' + tiles_folder + separator + designator + filename.replace('.ply',
                                                                                                        '.zip').replace(
                '.pts', '.zip').replace('.ex', '.zip').replace('.obj', '.zip').replace('.e57', '.zip') + "."
            self.write_to_log(path, separator, message)
        ...
        if resulting_mesh_type == "m":
            ...
            with zipfile.ZipFile(zip_file, mode="w") as zf:
                for ext in extensions:
                    ...
                    try:
                        zf.write(
                            with_texture_output_folder + separator + designator + filename.replace('.obj', '').replace(
                                '.ply', '').replace('.pts', '').replace('.ex', '').replace('.e57', '') + ext,
                            designator + filename.replace('.obj', '').replace('.ply', '').replace('.pts', '').replace(
                                '.ex', '').replace('.e57', '') + ext,
                ...
                zf.write(with_texture_output_folder + separator + designator + 'texture.png',
                         designator + 'texture.png', compress_type=compression, compresslevel=9)
    ...
    def open_obj_model(self, model_dest_folder):
        ...
        if 'tpc_' in model_dest_folder:
            ...
            o3d.visualization.draw([{"name": "Model", "geometry": model, "material": material}],
                                   title="ARTAK 3D Map Maker || LiDAR Edition", bg_color=[0, 0, 0, 0],
                                   show_skybox=False, actions=None, width=1280, height=900)
